...
public sealed class CategoryAuthorizationPipelineTests
{
    [Fact]
    public async Task HttpPipeline_WhenCategoryCreateIsUnauthenticatedOrBuyer_ReturnsUnauthorizedOrForbidden()
    {
        ...
        var missing = await client.PostAsync("/api/v1/categories", content);
        ...
        using var buyerRequest = new HttpRequestMessage(HttpMethod.Post, "/api/v1/categories")
        {
            Content = JsonContent.Create(new { name = "Fruit", slug = "fruit", displayOrder = 0 })
        };
