# Haggly Backend Architecture

This document describes the backend architecture currently present under
`backend/`. Root integration boundaries and frontend architecture are described
in `../ARCHITECTURE.md` and `../frontend/ARCHITECTURE.md`. Planned backend
modules and technologies are called out separately and must not be read as
...
The backend solution contains four production projects and one active test
project; the functional-test project is planned for a later testing phase:
...
The root `../README.md` remains the source for MVP scope and business requirements. This
