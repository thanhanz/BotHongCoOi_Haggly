...
public sealed class UpdateDailyProductListingHandler(
    IInventoryCommandRepository repository,
    IInventoryReferenceQuery references,
    IInventoryUnitOfWork unitOfWork,
    IBusinessClock businessClock)
    : IRequestHandler<UpdateDailyProductListingCommand, DailyProductListingDto>
{
    public async Task<DailyProductListingDto> Handle(
        UpdateDailyProductListingCommand command,
        CancellationToken cancellationToken)
    {
        ...
        return await unitOfWork.ExecuteInTransactionAsync(async transactionCancellationToken =>
        {
            ...
            try
            {
                if (command.PublicUnitPrice is decimal publicUnitPrice)
                {
                    if (listing.PublicUnitPrice != publicUnitPrice)
                    {
                        listing.ChangePrice(publicUnitPrice, command.ActorUserId, occurredAt);
                    }
                }

                if (command.Status is DailyListingStatus status)
                {
                    switch (status)
                    {
                        case DailyListingStatus.AVAILABLE:
                            listing.Show(command.ActorUserId, occurredAt);
                            break;
                        case DailyListingStatus.HIDDEN:
                            listing.Hide(command.ActorUserId, occurredAt);
                            break;
                        default:
