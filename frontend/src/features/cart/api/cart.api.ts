...
import type { AddCartItemRequest, Cart, CreatedOrder, CreateOrderRequest, UpdateCartItemRequest } from "./cart.contracts";

export function addCartItem(request: AddCartItemRequest, options: ApiRequestOptions = {}): Promise<Cart> {
  return apiRequest<Cart, AddCartItemRequest>({ method: "POST", url: "/cart/items", data: request, ...options });
}
